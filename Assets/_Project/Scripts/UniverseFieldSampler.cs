...
public class UniverseFieldSampler : MonoBehaviour
{
    ...
    public UniverseSample Sample(Vector3 position, float simulationTime)
    {
        ...
        // Cosmic drift due to dark energy
        ...
        // Multi-scale density field (cosmic web → filament → cluster)
        ...
        float density = Mathf.Clamp01(
            (web * 0.55f + filament * 0.3f + cluster * 0.15f)
            * constants.gravity / (0.65f + constants.darkEnergy * 0.2f));
    ...
    public PlanetArchetype ClassifyPlanet(UniverseConstants c, float density, float radiation, float entropy, float life)
    {
        if (c.gravity    > 6f && density  > 0.7f) return PlanetArchetype.Collapsed;
        if (c.darkEnergy > 3.5f && density < 0.3f) return PlanetArchetype.ThinExiled;
        if (entropy > 0.72f)                       return PlanetArchetype.Volatile;
        if (life > 0.58f && radiation > 0.25f && radiation < 0.65f) return PlanetArchetype.Habitable;
